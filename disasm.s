...
; External Routines *removed these routines as I am importing 'disasm.s" into ewoz
...
SpaceOrEscape:
  ...
  CMP #ESC
  BEQ PROMPT
  CMP #'Q'    ; added this option to quit if needed.
  JMP RESET
  BNE SpaceOrEscape
...
; Destination address is Current address + relative (sign extended so upper byte is $00 or $FF) + 3
  ...
  BMI NEG1              ; if negative, need to sign extend
...
; Get character from keyboard
GetKey:
  JSR GETCHAR    ; Get char  from keyboard.
  RTS
...
ContinueString:	.byte CR, "  <SPACE > TO CONTINUE, < ESC > TO STOP, < Q > TO QUIT", $0
WelcomeString:	.byte "DISASM VERSION 1.0 BY JEFF TRANTER",$0
PromptString:	.byte CR, "ADDRESS? ",$0
